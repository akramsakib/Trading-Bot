...
def get_market_snapshot() -> pd.DataFrame:
    ...
    # Masks the key so it's safe to appear in GitHub Actions logs.
    key_len = len(TWELVE_DATA_API_KEY) if TWELVE_DATA_API_KEY else 0
    masked_url = url.replace(TWELVE_DATA_API_KEY, f"***KEY_LEN_{key_len}***")
    print(f"DEBUG: requesting URL: {masked_url}")
    # --- END DEBUG OUTPUT ---
